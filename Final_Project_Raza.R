...
ui <- fluidPage(
  theme = shinytheme("flatly"), # Apply a modern theme for a professional look
    ...
    mainPanel(
      tabsetPanel(
        ...
        tabPanel("Plot",
                 ...
                 #download PNG for the main plot
        ...
        tabPanel("Compare Models",
                 ...
                 #download CSV for the metrics table
                 br(),
                 downloadButton("dl_metrics", "Download metrics (CSV)")
        ),
        # ---- Minimal Bootstrap tab (point estimates only) ----
        tabPanel("Bootstrap",
...
server <- function(input, output, session) {
  ...
  #NEW: Huber (BFGS) helpers (summary only)
  ...
  results <- eventReactive(input$run, {
    ...
    #fit Huber (BFGS) for summary only
  ...
  # Compare Models table (AIC removed)
  ...
  # ===== Minimal Bootstrap (POINT ESTIMATES ONLY) =====
  ...
  # Table with ONLY Model, Term, Estimate
  boot_coef_table_data <- reactive({
    br <- boot_res()
